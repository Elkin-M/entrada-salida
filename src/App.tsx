...
import { Scan, User, Laptop, CheckCircle, AlertTriangle, XCircle, Plus, Loader2, LogIn, LogOut, UserCheck, QrCode, ArrowRight, Download, FileText, Camera, X, Keyboard, Grid, Barcode, Zap } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    Quagga.init({
      ...
      if (err) console.error("Error inicializando Quagga:", err);
    });
  }, []);
  ...
  const handleImageScan = async (imageBlob: Blob) => {
    setIsScanning(true);
    try {
      // Intenta con el API externo (solo QR)
      ...
      // Si no es QR, intenta con Quagga (para códigos de barras)
  ...
  const processScanCode = (code: string) => {
    ...
    if (appState === STATE.WAITING || appState === STATE.RESULT) {
        fetchUser(code);
    } else if (appState === STATE.USER_DETECTED) {
        validateAsset(code);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        if (data && data[0]?.symbol?.[0]?.data) {
          ...
          // Si no es QR, intenta con Quagga
          ...
          Quagga.decodeSingle({
            ...
            if (result && result.codeResult) {
              setIsScanning(false);
              processScanCode(result.codeResult.code);
            } else {
              setIsScanning(false);
              alert("No se detectó código QR ni de barras.");
            }
          });
        }
      ...
      if (data.status === 'success') {
        ...
        if (isDashboardSearch) {
            ...
            if (data.tipo === 'ACTIVO') {
                ...
                showFeedback('success', `${currentMode} RÁPIDA: ${data.usuario.nombre}`);
            } else {
  ...
  const handleManualEntry = async () => {
    setAppState(STATE.LOADING);
    const success = await logMovement(currentMode, currentUser.cedula, 'PEATONAL', 'EXITOSO', currentUser.nombre);
  ...
  const registerNewUserAndAsset = async () => {
    ...
    try {
      ...
      if (data.status === 'success') {
        await logMovement('ENTRADA', currentUser.cedula, payload.id_activo, 'EXITOSO (REGISTRO)', newUserForm.name);
  ...
  const finalImageUrl = getCodeUrl();
  const screenQrUrl = generatedCodeData ? `https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${encodeURIComponent(finalImageUrl)}` : '';
    ...
    <div className={`min-h-screen ${getBgColor()} transition-colors duration-500 text-white font-sans overflow-hidden flex flex-col items-center justify-center p-4`}>
      ...
      {showCamera && (
          <div className="fixed inset-0 bg-black/95 z-50 flex flex-col items-center justify-center">
              ...
              <div className="bg-slate-800 px-6 py-2 rounded-full flex items-center gap-2">{nativeMode ? <><Zap className="w-4 h-4 text-green-400" /><span className="text-green-400 font-bold text-sm">Modo Nativo (Barras y QR)</span></> : <span className="text-yellow-400 font-bold text-sm">Modo Web (QR y Barras)</span>}</div>
          </div>
      )}

      <div className="absolute top-4 left-4 flex items-center gap-4 opacity-80">
        <div className="flex items-center gap-2"><Scan className="w-6 h-6" /><span className="font-mono text-sm">GATEKEEPER v16.3</span></div>
      </div>
                      ...
                      <div className="animate-fadeIn">
                          <div className="flex items-center gap-4 bg-slate-800/50 p-4 rounded-xl mb-6 border border-slate-700">
                              ...
                              <div className="flex-1">
                                  <h3 className="text-xl font-bold text-white">{String(currentUser.nombre)}</h3>
                                  <p className="text-slate-400 font-mono">CC: {String(currentUser.cedula)}</p>
                              </div>
